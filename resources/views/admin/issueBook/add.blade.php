...
@section('content')
<div class="">
    <h1 class="d_title">Book Issue</h1>
</div>
    ...
    <form action="{{route('admin.store.book-issue')}}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-2 gap-4">
            <div class="form_group">
                ...
                <select name="bookID" class="form_input @error('bookID') is-invalid @enderror" id="bookID" aria-describedby="titleHelp">
                    ...
                    @foreach ($books as $book)
                        <option value="{{$book->id}}" @if (old('bookID') == $book->id) selected @endif >{{$book->bookName}}({{$book->id}})</option>
                    @endforeach
                </select>
                @error('bookID')
            ...
            <div class="form_group">
                ...
                <select name="studentID" class="form_input @error('studentID') is-invalid @enderror" id="studentID" aria-describedby="titleHelp">
                    ...
                    @foreach ($students as $student)
                        <option value="{{$student->id}}" @if (old('student') == $student->id) selected @endif >{{$student->studentName }}({{$student->id}})</option>
                    @endforeach
                </select>
                @error('studentID')
                    <div class="text-red-500">{{ $message }}</div>
            ...
            <div class="form_group">
                ...
                <input disabled readonly type="date" name="issueDate" class="form_input bg-gray-200 @error('issueDate') is-invalid @enderror" id="issueDate" value="{{date('Y-m-d')}}" >
                @error('issueDate')
                    <div class="text-red-500">{{ $message }}</div>
                @enderror
            </div>
            <div class="form_group">
                ...
                <input type="date" name="returnDate" class="form_input @error('returnDate') is-invalid @enderror" id="returnDate" aria-describedby="titleHelp" value="{{old('returnDate')}}">
                @error('returnDate')
                    <div class="text-red-500">{{ $message }}</div>
                @enderror
            </div>
